...
class EnhancedFirebaseService:
    ...
    def get_users_by_role(self, role: str, include_inactive: bool = False) -> List[Dict]:
        ...
        """
        try:
            query = self.db.collection('users').where('role', '==', role)
    ...
    def archive_audit_logs_to_storage(self, days_old: int = 90) -> int:
        ...
        try:
            ...
            query = self.db.collection('auditLogs').where(
            ...
            blob = self.storage_client.blob(f"audit_archives/{archive_filename}")
    ...
    def store_ml_model(self, model_name: str, model_data: bytes, metadata: Dict) -> str:
        ...
        try:
            ...
            blob = self.storage_client.blob(model_path)
            ...
            self.db.collection('mlModels').add(model_metadata)

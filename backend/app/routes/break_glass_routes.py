...
async def submit_emergency_request():
    ...
    try:
        ...
        result = await enhanced_break_glass_service.submit_emergency_request(request_data)
...
async def get_available_administrators():
    ...
    try:
        ...
        administrators = await service.get_available_administrators()
...
@require_role(['admin'])
@handle_break_glass_errors
async def get_pending_emergency_requests():
    """
    ...
    try:
        ...
        pending_requests = await service.get_pending_emergency_requests(request.user_id)
...
@require_role(['admin'])
@handle_break_glass_errors
async def approve_emergency_request(request_id):
    """
    ...
    try:
        ...
        result = await enhanced_break_glass_service.process_approval(request_id, request.user_id, 'approved', comments)
...
@require_role(['admin'])
@handle_break_glass_errors
async def deny_emergency_request(request_id):
    """
    ...
    try:
        ...
        # Use enhanced break-glass service
        result = await enhanced_break_glass_service.process_approval(request_id, request.user_id, 'denied', comments)
...
@require_role(['admin'])
@handle_break_glass_errors
async def monitor_emergency_session(session_id):
    """
...
@require_auth
@handle_break_glass_errors
async def log_emergency_activity(session_id):
    """
    ...
    try:
        ...
        # Log activity
        success = await service.log_emergency_activity(session_id, activity_data)
...
@require_auth
@handle_break_glass_errors
async def get_emergency_request(request_id):
    """
    ...
    try:
        ...
        requester_info = await service._get_user_info(request_data['requesterId'])
        if requester_info:
        ...
        for resource_id in request_data.get('requiredResources', []):
            resource_info = await service._get_resource_segment_info(resource_id)
            if resource_info:
...
async def get_emergency_sessions():
    ...
    try:
        ...
        for doc in query.stream():
            ...
            from ..services.break_glass_service import get_break_glass_service
            service = get_break_glass_service(db)
            user_info = await service._get_user_info(session_data['userId'])
            if user_info:
...
@require_role(['admin'])
@handle_break_glass_errors
async def get_emergency_report(session_id):
    """
...
@require_role(['admin'])
@handle_break_glass_errors
async def submit_post_incident_review(session_id):
    """
    ...
    try:
        ...
        await service._log_post_incident_review(session_id, request.user_id, review_data)
...
@require_role(['admin'])
@handle_break_glass_errors
async def generate_comprehensive_report(session_id):
    """
    ...
    try:
        ...
        report_data = await service.generate_comprehensive_report(session_id)
...
@require_role(['admin'])
@handle_break_glass_errors
async def get_off_hours_statistics():
    """
    ...
    try:
        ...
        statistics = await enhanced_break_glass_service.get_off_hours_statistics(days)
...
@require_role(['admin'])
@handle_break_glass_errors
async def get_senior_administrators():
    """
    ...
    """
    try:
        senior_admins = await enhanced_break_glass_service._get_senior_administrators()
...
@require_role(['admin'])
@handle_break_glass_errors
async def test_off_hours_alerts():
    """
    ...
    try:
        ...
        # Send test alert
        alert_sent = await enhanced_break_glass_service._alert_senior_administrators(
            'test-request-id',
            test_request_data
        )
